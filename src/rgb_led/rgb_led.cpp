...
void Rgb_Led::setRed(uint8_t value) {
    uint16_t redDutyCycle = map(value, 0, 4095, 0, this->RED_MAX_DUTY_CYCLE);
    ledcWrite(redPWMChannel, redDutyCycle);
}

void Rgb_Led::setGreen(uint8_t value) {
    uint16_t greenDutyCycle = map(value, 0, 4095, 0, this->GREEN_MAX_DUTY_CYCLE);
    ledcWrite(greenPWMChannel, greenDutyCycle);
}

void Rgb_Led::setBlue(uint8_t value) {
    uint16_t blueDutyCycle = map(value, 0, 4095, 0, this->BLUE_MAX_DUTY_CYCLE);
